...
import { renderTemplate } from './lib/template/template.js';
...
app.get('/', async (_, res) => {
	const html = renderTemplate(fileGroups, "index.html");
	res.status(200).type('html').end(html);
})

app.get('/feed-:slug.xml', async (req, res) => {
	const s = renderTemplate(fileGroups.filter(fg => {
		return fg.slug === req.params.slug
	}), "feed.xml");
	res.status(200).type('application/rss+xml').end(s);
})
